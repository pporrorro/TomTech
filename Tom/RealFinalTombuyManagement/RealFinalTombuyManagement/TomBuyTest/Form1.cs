...
namespace TomBuyTest
{
    public partial class Form1 : Form
    {
        ...
        private void btnShowOrderbasket_Click(object sender, EventArgs e)//출고보내기버튼
        {
            //출고 가능량 체크하기
            ...
            if (iKSalePrice > 0)
            {
                sOrderList += $"마스크의 출고개수 : {numCount1.Value} , 판매 금액 : {iKSalePrice} :  \r\n";
                sOrderList += $"마스크의 누적 출고개수 : {pi.oiKLeftC}, 마스크의 누적 판매금액 :{iKSalePrice * pi.oiKLeftC}  \r\n";
            ...
            if (iWSalePrice > 0)
            {
                sOrderList += $"물티슈의 출고개수 : {numCount2.Value} , 판매 금액 : {iWSalePrice} :  \r\n";
                sOrderList += $"물티슈의 누적 출고개수 : {pi.oiWLeftC}, 물티슈의 누적 판매금액 :{iWSalePrice * pi.oiWLeftC}  \r\n";

                isOrder = true;
            }
            if (iESalePrice >  0)
            {
                sOrderList += $"이어폰의 출고개수 : {numCount3.Value} , 판매 금액 : {iESalePrice} :  \r\n";
                sOrderList += $"이어폰의 누적 출고개수 : {pi.oiELeftC}, 이어폰의 누적 판매금액 :{iESalePrice * pi.oiELeftC}  \r\n";

                isOrder = true;
            }
            if (iASalePrice > 0)
            {
                sOrderList += $"방향제의 출고개수 : {numCount4.Value} , 판매 금액 : {iASalePrice} :  \r\n";
                sOrderList += $"방향제의 누적 출고개수 : {pi.oiALeftC}, 방향제의 누적 판매금액 :{iASalePrice * pi.oiALeftC}  \r\n";

                isOrder = true;
            }
            if (iBSalePrice > 0)
            {
                sOrderList += $"건전지의 출고개수 : {numCount5.Value} , 판매 금액 : {iBSalePrice} :  \r\n";
                sOrderList += $"건전지의 누적 출고개수 : {pi.oiBLeftC}, 건전지의 누적 판매금액 :{iBSalePrice * pi.oiBLeftC}  \r\n";
